...
FreqRec* break_into_parts_and_find_lead_freqs(const freq_t freq_window, const ms_t division_time_ms, uint32_t *_freqs_n,
                                              const float* pcm_buffer, const smpn_t sample_rate, const smpn_t samples_n)
{
    ...
    const uint32_t freqs_n = samples_n / samples_per_part; // number of freq recs in result table
    
    const uint32_t fft_buffer_len = freq_window * 2;
 
    kiss_fft_cpx* fft_buffer = (kiss_fft_cpx*) malloc(sizeof(kiss_fft_cpx) * fft_buffer_len);
    ...
    FreqRec *freqs = (FreqRec*) malloc(sizeof(FreqRec) * freqs_n);
    ...
    // The idea behind this is in breaking original pcm data in parts (i.e. by 10ms),
    // and finding most powerful freq in spectrum
    for (uint32_t part_index = 0; part_index < freqs_n; part_index++)
    {
